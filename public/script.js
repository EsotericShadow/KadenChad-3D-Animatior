...
characters to life`;
                ...
                if (prevThresholdIndex !== currentThresholdIndex) {
                    ...
                    if (this.softLockEnabled) {
                        ...
                        setTimeout(() => {
                            ...
                            // Reset touch tracking
                            this.touchStartZoom = this.targetZoomZ;
                            this.touchStartDistance = currentDistance;
                        }, 1000);
                    }
                }
    ...
    handleUserInteraction() {
        ...
        } else if (this.zoomZ > -1500) {
            this.activeFrameIndex = 1; // Home
        } else if (this.zoomZ > -2500) {
            this.activeFrameIndex = 2; // Bio
        } else if (this.zoomZ > -3500) {
            this.activeFrameIndex = 3; // Reel
        } else {
            this.activeFrameIndex = 4; // Contact
        }
        
        // Only update if the active frame has changed
        if (prevActiveFrameIndex !== this.activeFrameIndex) {
            this.updateNavLinks();
    ...
    updateNavLinks() {
        // Map the active frame index to the corresponding nav link index
        let navIndex = this.activeFrameIndex;
        if (navIndex === 1) navIndex = 0; // Both logo and home frames map to the Home nav link
        
        document.querySelectorAll('.nav-link').forEach(link => {
            const linkIndex = parseInt(link.dataset.index);
            if (linkIndex === navIndex) {
    ...
    updateFrameVisibility() {
        // Update frame visibility based on active frame index
        this.frames.forEach((frame, index) => {
            const element = document.getElementById(frame.id);
            if (index === this.activeFrameIndex) {
                element.classList.add('frame-visible');
                element.classList.remove('frame-exiting');
                
                // Add locked class if soft-locking is enabled
                if (this.softLockEnabled) {
                    element.classList.add('frame-locked');
                    setTimeout(() => {
                        element.classList.remove('frame-locked');
                    }, 2000);
                }
            } else if (index === this.previousFrameIndex) {
                element.classList.remove('frame-visible');
                element.classList.add('frame-exiting');
            } else {
                element.classList.remove('frame-visible', 'frame-exiting', 'frame-locked');
            }
        });
    ...
    createContentFrames() {
        ...
        bioFrame.innerHTML = `
            ...
            <p>${this.bioText}</p>
            <a href="#" class="badge">Hear My Story</a>
        `;
        ...
        reelFrame.id = 'reel-frame';
        reelFrame.className = 'frame';
        reelFrame.innerHTML = `
            <h2>Animation Reel</h2>
            <div class="video-container">
                <video controls>
                    <source src="assets/New_Demo.mp4" type="video/mp4">
    ...
    animate() {
        ...
        if (this.particleSystem) {
            ...
            for (let i = 0; i < positions.length; i += 3) {
                // Apply subtle sine wave motion
                positions[i] += Math.sin(time + positions[i] * 0.01) * 0.1;
                positions[i + 1] += Math.cos(time + positions[i + 1] * 0.01) * 0.1;
                
                // Reset particles that go too far
                if (positions[i] > 1500) positions[i] = -1500;
                if (positions[i] < -1500) positions[i] = 1500;
                if (positions[i + 1] > 1500) positions[i + 1] = -1500;
                if (positions[i + 1] < -1500) positions[i + 1] = 1500;
            }
        ...
        const now = Date.now();
        if (now - this.lastScrollTime > 5000 && this.tooltipVisible) {
