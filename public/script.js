...
characters to life`;
    ...
    setupEventListeners() {
        ...
        // Direct navigation setup - simplified and more reliable
        ...
        // Add direct click handlers to navigation links immediately
        const navLinks = document.querySelectorAll('.nav-link');
        navLinks.forEach(link => {
            link.onclick = (e) => {
                e.preventDefault();
    ...
    navigateToSection(index) {
        ...
        if (this.isTransitioning) {
            console.log("Navigation blocked - currently transitioning");
            return;
        }
        
        this.isTransitioning = true;
        
        // Map the navigation indices to the correct zoom thresholds
        // 0 = Home (-500), 1 = Bio (-1500), 2 = Reel (-2500), 3 = Contact (-3500)
        const zoomPositions = {
            0: -500,   // Home
            1: -1500,  // Bio
            2: -2500,  // Reel
        ...
        this.targetZoomZ = zoomPositions[index] || -500;
        this.activeFrameIndex = index;
        
        console.log("Setting target zoom to: ", this.targetZoomZ);
    ...
    updateNavLinks() {
        // Map the threshold indices to navigation indices
        // 0 = Home, 1 = Bio, 2 = Reel, 3 = Contact
        const thresholdToNavMap = {
            0: 0, // Logo/Home
            1: 0, // Home
            2: 1, // Bio
            3: 2, // Reel
        ...
        const navIndex = thresholdToNavMap[this.activeFrameIndex] || 0;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.nav-link').forEach(link => {
        link.onclick = (e) => {
            e.preventDefault();
            const index = parseInt(link.getAttribute('data-index'));
            console.log("Direct navigation click: ", index);
            if (window.portfolioScene) {
                window.portfolioScene.navigateToSection(index);
            }
        };
    });
});
